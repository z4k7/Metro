...
const loadProduct = async (req, res, next) => {
  try {
    ...
    const pdtsData = await Products.find({name:{$regex:''+ searchQuery,$options:'i'}}).populate("category").populate('offer').sort({ createdAt: -1 }).skip( (pageNum - 1)*limit ).limit(limit);
