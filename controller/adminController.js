...
const getDashboard = async (req, res, next) => {
  try {
    ...
    const usersOntheMonth = formatNum(
      await User.find({ createdAt: { $gte: firstDayOfMonth } }).count()
    );
    ...
    let monthName = "";
    if (req.query.month) {
      (salesMonth = "Weeks"), (monthName = getMonthName(req.query.month));
    ...
    if (req.query.year && req.query.month) {
      orderData.forEach((year) => {
        months.push(`Week ${year._id.createdAt}`);
      });
    ...
    const categoryData = await Orders.aggregate([
      ...
      },
      {
        $group: {
          ...
          sales: { $sum: "$totalPrice" }, // Assuming 'name' is the field you want from the category collection
        },
      },
    ]);
    ...
    let orderDataToDownload = await Orders.find({ status: "Delivered" }).sort({
      createdAt: 1,
    });
    if (req.query.fromDate && req.query.toDate) {
      const { fromDate, toDate } = req.query;
      orderDataToDownload = await Orders.find({
        status: "Delivered",
