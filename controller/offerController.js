...
const loadOffer = async (req, res, next) => {
  try {
    ...
    res.render("offers", { offers, adminData });
  } catch (error) {
    next(error);
  }
};
...
const loadAddOffer = async (req, res, next) => {
  try {
    const adminId = req.session.admin;
    const adminData = await Admin.findOne({ _id: adminId });
    res.render("addOffer,adminData", adminData);
  } catch (error) {
    next(error);
  }
};
...
const postAddOffer = async (req, res, next) => {
  try {
    ...
    if (
      new Date(startingDate) >= new Date(expiryDate) ||
...
const loadEditOffer = async (req, res, next) => {
  try {
    ...
    res.render("editOffer", { offerData, adminData });
  } catch (error) {
    next(error);
  }
};
